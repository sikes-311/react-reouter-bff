...
指示:
  1. frontend/e2e/{feature}.spec.ts に対象シナリオのテストを追加する（.feature ファイルは作成しない）
...
| #1.5 | team-lead | `bash scripts/generate-api-types.sh` を実行して openapi.json と型を生成 | #1完了後 |
| #2 | frontend-agent | フロントエンド実装（loader / action / components / api.ts） | #1.5完了後 |
...
**B-1.5: openapi.json・型生成（team-lead が実行）**

bff-agent の完了報告を受けたら、以下を実行してください:
...
生成物:
- `docs/api/openapi.json` — BFF の OpenAPI スペック
- `frontend/types/generated/api.d.ts` — TypeScript 型定義（自動生成）

frontend-agent への指示には以下を追記すること:
- **手動型定義は書かないこと**。`types/generated/api.d.ts` の生成型を `import type { XxxResponse } from "~/types/generated/api"` で使用する
- `api.ts` の型定義もこのファイルから import する
